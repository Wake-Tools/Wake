...
*/
typedef struct sapp_framed_touchpoint{int x;int y; int id;int state;}sapp_framed_touchpoint;
SOKOL_APP_API_DECL int wsapp_get_multitouch_frame(sapp_framed_touchpoint** _out);
...
SOKOL_APP_API_DECL void wsapp_set_create_window_style(uint32_t win_ex_style, uint32_t win_style);//[Patch] 
SOKOL_APP_API_DECL void wsapp_wait_framerate(float sec, unsigned char swap_interval);//[Patch] 
SOKOL_APP_API_DECL const char* sapp_get_clipboard_string(void);
/* set the window title (only on desktop platforms) */
SOKOL_APP_API_DECL void wsapp_set_win32_wndproc_cb(void*/*WNDPROC*/ callback);/* [patch] note: return -1 to process other sapp win message */ 
typedef int (*win_fptr)(HWND);
SOKOL_APP_API_DECL void wsapp_set_window_created_cb(win_fptr callback);/* [patch] */ 
...
DXGI_SWAP_CHAIN_DESC * sc_desc = (DXGI_SWAP_CHAIN_DESC *)&_sapp.d3d11.swap_chain_desc;
...
_SOKOL_PRIVATE void _sapp_d3d11_destroy_device_and_swapchain(void) {_sapp_d3d11_destroy_device_and_swapchain_dcomp();
...
DWORD win_ex_style = WS_EX_NOREDIRECTIONBITMAP;
DWORD win_style;
RECT rect = { 0, 0, 0, 0 };
...
WNDPROC win32_wndproc_cb = 0;
SOKOL_API_IMPL void wsapp_set_win32_wndproc_cb(void* callback) { win32_wndproc_cb = (WNDPROC)callback;}
...
if (win32_wndproc_cb) { int ret = win32_wndproc_cb(hWnd, uMsg, wParam, lParam); if (ret != -1) return ret; }
switch (uMsg) {
PATCH_SOKOL_XTR_WIN_MSG
case WM_CLOSE:
...
DWORD xtra_win_ex_style=0;DWORD xtra_win_style=0;SOKOL_API_IMPL void wsapp_set_create_window_style(uint32_t win_ex_style, uint32_t win_style) {xtra_win_ex_style = win_ex_style;xtra_win_style = win_style;}//[Patch]
win_fptr win_created_cb=0;
SOKOL_API_IMPL void wsapp_set_window_created_cb(win_fptr callback){win_created_cb=callback;}
...
if(xtra_win_ex_style)win_ex_style=xtra_win_ex_style;if(xtra_win_style)win_style=xtra_win_style;
...
if(win_created_cb)win_created_cb(_sapp.win32.hwnd);//[Patch]
...
sapp_set_icon(&desc->icon);
#if defined(SOKOL_D3D11)
_sapp_d3d11_create_device_and_swapchain_dcomp();
_sapp_d3d11_create_default_render_target();
#endif
#if defined(SOKOL_GLCORE)
_sapp_wgl_init();
...
_sapp_frame();
if(_sapp.swap_interval<=5){
#if defined(SOKOL_D3D11)
if(_sapp.swap_interval<5)_sapp_d3d11_present(false);
